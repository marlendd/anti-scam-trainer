...
function HighlightedDescription({
  ...
  redFlag: ScenarioRedFlag
}) {
  if (
    !redFlag.accent ||
    !redFlag.description.includes(redFlag.accent)
  ) {
    return <>{redFlag.description}</>
  }

  const [before, after] = redFlag.description.split(
    redFlag.accent,
    2,
  )
...
export function ScenarioAnalysisPanel({
  ...
  const rootClassName = [styles.panel, className]
    .filter(Boolean)
    .join(' ')
    ...
    <aside className={rootClassName}>
      ...
      <div className={styles.redFlags}>
        {analysis.redFlags.map((redFlag) => (
          ...
          >
            <h3 className={styles.redFlagTitle}>
              {redFlag.title}
            </h3>
