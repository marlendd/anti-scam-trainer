...
export function Logo({ alt = 'Антискам-тренажёр', className, ...props }: LogoProps) {
    const classes = [styles.image, className].filter(Boolean).join(' ')

    return (
        <picture className={styles.logo}>
            <source media="(max-width: 860px)" srcSet={logoSmall} />

            <source media="(max-width: 1100px)" srcSet={logoDefault} />

            <img {...props} className={classes} src={logoLarge} alt={alt} />
        </picture>
    )
}
