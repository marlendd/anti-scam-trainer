...
export type ScenarioPlaybackMode = 'preview' | 'interactive'
...
export function useScenarioPlayback({
    ...
    const currentChoice = currentItem?.type === 'choice' ? currentItem : null

    const isCompleted = currentIndex >= timeline.length

    const appendAnswer = useCallback(
        (choiceId: string, answer: ScenarioAnswerOption) => {
    ...
    useEffect(() => {
        ...
        if (currentItem.type === 'choice') {
            ...
            const previewAnswer =
                currentItem.options.find((option) => option.id === currentItem.previewOptionId) ??
            ...
            const timeout = window.setTimeout(() => {
                appendAnswer(currentItem.id, previewAnswer)
            }, 500)
        ...
        const timeout = window.setTimeout(() => {
            setVisibleMessages((current) => [...current, currentItem])
    ...
    }, [appendAnswer, currentItem, isCompleted, mode])
