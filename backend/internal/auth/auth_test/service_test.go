...
type userRepositoryStub struct {
	createFn         func(ctx context.Context, email, passwordHash string) (auth.User, error)
...
func (s *userRepositoryStub) Create(ctx context.Context, email, passwordHash string) (auth.User, error) {
	if s.createFn == nil {
...
func TestService_Register(t *testing.T) {
	...
	t.Run("successful registration hashes password before storing", func(t *testing.T) {
		...
		users := &userRepositoryStub{
			createFn: func(_ context.Context, email, passwordHash string) (auth.User, error) {
				require.Equal(t, "test@example.com", email)
				capturedHash = passwordHash
				return auth.User{ID: "user-1", Email: email, PasswordHash: passwordHash}, nil
			},
		}
		...
		u, err := service.Register(context.Background(), "test@example.com", "password123")
	...
	t.Run("returns ErrEmailTaken when repository reports duplicate", func(t *testing.T) {
		...
		users := &userRepositoryStub{
			createFn: func(context.Context, string, string) (auth.User, error) {
		...
		_, err := service.Register(context.Background(), "taken@example.com", "password123")
...
func TestService_Login(t *testing.T) {
	...
	t.Run("successful login creates a session", func(t *testing.T) {
		...
		existingUser := auth.User{ID: "user-1", Email: "test@example.com", PasswordHash: hash}
		...
		sessions := &sessionRepositoryStub{
			createFn: func(_ context.Context, userID, userAgent, ip string, ttl time.Duration) (auth.Session, error) {
				...
				return auth.Session{ID: "session-1", UserID: userID, ExpiresAt: time.Now().Add(ttl)}, nil
			},
		}

		service := newTestService(users, sessions)

		u, sess, err := service.Login(context.Background(), "test@example.com", "password123", "test-agent", "127.0.0.1")
	...
	t.Run("returns ErrInvalidCredentials when user not found", func(t *testing.T) {
		...
		_, _, err := service.Login(context.Background(), "nobody@example.com", "password123", "", "")
	...
	t.Run("returns ErrInvalidCredentials on wrong password without creating session", func(t *testing.T) {
		...
		users := &userRepositoryStub{
			getByEmailFn: func(context.Context, string) (auth.User, error) {
				return auth.User{ID: "user-1", PasswordHash: hash}, nil
			},
		}
		sessions := &sessionRepositoryStub{
		...
		_, _, err = service.Login(context.Background(), "test@example.com", "wrong-password", "", "")
...
func TestService_GetUserBySession(t *testing.T) {
	...
	t.Run("returns user for active session", func(t *testing.T) {
		...
		expectedUser := auth.User{ID: "user-1", Email: "test@example.com"}
	...
	t.Run("expired session is deleted and returns ErrSessionNotFound", func(t *testing.T) {
		...
		sessions := &sessionRepositoryStub{
			getByIDFn: func(context.Context, string) (auth.Session, error) {
				return auth.Session{
					ID:        "session-1",
					UserID:    "user-1",
					ExpiresAt: time.Now().Add(-time.Hour), // уже истекла
				}, nil
			},
		}
		users := &userRepositoryStub{}
		...
		require.Equal(t, []string{"session-1"}, sessions.deleteCalledWith, "expired session must be cleaned up")
	})
	...
	t.Run("returns ErrSessionNotFound when session does not exist", func(t *testing.T) {
		...
		_, err := service.GetUserBySession(context.Background(), "missing-session")
...
func (s *passwordResetRepositoryStub) Create(context.Context, string, string, time.Duration) (auth.PasswordResetToken, error) {
	return auth.PasswordResetToken{}, nil
}

func (s *passwordResetRepositoryStub) GetByTokenHash(context.Context, string) (auth.PasswordResetToken, error) {
	return auth.PasswordResetToken{}, nil
}

func (s *passwordResetRepositoryStub) MarkUsed(context.Context, string) error {
	return nil
}

func newTestService(users auth.UserRepository, sessions auth.SessionRepository) *auth.Service {
	...
	return auth.NewService(users, sessions, &passwordResetRepositoryStub{}, m, "http://localhost:3000")
}
